...
static void *thread_batch_io(void *data)
{
    ...
    CUfileBatchHandle_t batch_id = t->batch_id;
    CUfileIOParams_t *io_batch_params = t->io_batch_params;
    int i; // = t->i;
    int j = t->j; //batch number 
    int batch_offset = t->batch_offset;
    int size = t->size;
    ...
    for(i = 0; i < batch_size; i++) {
        io_batch_params[i].u.batch.file_offset = batch_offset*j + i * size;
    }
    //std::cout << "Batch "<< j <<" file offset set " << std::endl;
    //std::cout << "Batch "<< j << " batch offset = " << batch_offset <<std::endl;
    CUfileError_t errorBatch = cuFileBatchIOSubmit(batch_id, batch_size, io_batch_params, flags);
    if(errorBatch.err != 0) {
